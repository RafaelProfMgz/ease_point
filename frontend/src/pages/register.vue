        ...
        <v-card class="pa-8 glass-card" elevation="0" rounded="xl">
          ...
          <v-form @submit.prevent="handleRegister">
            ...
            <v-row dense>
              ...
              <v-col cols="12" sm="4">
                <!-- Select simplificado (Array de Strings) para evitar erros do Vue -->
                <v-select
                  v-model="form.plan"
...
const form = reactive({
  companyName: '',
  plan: 'Bronze', // Valor padrão (Capitalizado para bater com o select)
  adminName: '',
  email: '',
  password: ''
});
...
const handleRegister = async () => {
  ...
  // Tratamento de dados para o backend (Bronze -> bronze)
  const payload = {
    ...form,
...
/* 2. Blobs de fundo (IMPORTANTE: pointer-events: none) */
.bg-blob {
  position: absolute;
  border-radius: 50%;
  filter: blur(80px);
...
.blob-3 { width: 250px; height: 250px; background: #06b6d4; top: 40%; left: 60%; opacity: 0.2; }

/* 3. Z-Index para conteúdo */
.z-index-1 {
  position: relative;
  z-index: 1;
}
